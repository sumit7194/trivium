...
| **M6** ✗ **KILLED 2026-07-26 by its own gate** — the prior-art sweep was the admission condition, and it is not passed. **Half 1** ("emit ⟺ span") is stated in Oellerich & Emelianenko [arXiv:2403.04889](https://arxiv.org/abs/2403.04889) as an explicit failure trichotomy, on *the same instrument* (library→design matrix→SVD→near-null σ), **with a noise-calibrated cutoff `σ=√(Np)‖ε‖^{2/3}` (Cor. 4.2) and a spectral-gap library criterion — both sharper than our hand-set τ=1e-6**. **Half 2** (O4) is the target of Ray 2026 [arXiv:2603.20474](https://arxiv.org/abs/2603.20474), whose pipeline already contains log-basis Lasso *and* a constancy-gate + diversity-filter false-positive guard (FDR=0.0/9 systems). tabula independently arrived at the same held-out guard (§161, architectural) — three independent arrivals ⇒ settled practice. **R2's internal cross-gate value is untouched** (never a novelty claim). Payout: three adopted instrument upgrades; and O4 is relocated from "an obstruction we found" to "our threshold was the wrong kind." [falsification/M6_prior_art_gate](falsification/M6_prior_art_gate) · ~~R2 carried to a full write-up if it proves:~~ "what a linear-probe emit engine can and cannot read" — theorem + the G2/R1 measurements as its experimental section | the family's first candidate for a genuinely publishable *methods* result; prior-art sweep is the gate |
