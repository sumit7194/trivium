...
HALF, STEP = 0.08, 0.00025   # HALVED step: one edge instead of two, so this matches
# the treatment deltas on ORBIT COUNT (~320) rather than on x0 spacing. The control was the only
# delta at n=50 while every other ran ~100, and every absolute-scale statement in FINDINGS leans
# on that row -- see forward-look item 2.
OUT = Path(__file__).resolve().parent.parent / "results" / "g3_control.json"
