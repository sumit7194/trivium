...
if __name__=="__main__":
    ...
    for mode in ["varied","pinned"]:
        ...
        F0=F0[:,keep]   # RAW -- no column normalisation, reproducing 161 as-run:
        # wildly different dynamic ranges straight into the eigenproblem, so SVD conditioning
        # becomes the AGGRESSIVE intervention it is in tabula 161 rather than the mild one
        # my normalised baseline made it. This is the variant that actually tests their fix.
        res={}
    ...
    (OUT/"t2_raw.json").write_text(json.dumps(rep,indent=1))
