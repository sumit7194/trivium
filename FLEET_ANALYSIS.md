...
| R-2 | ansatz "no harmonic potential with cubic integral" | open — rests on `high_rank_killing`'s SW-IV σ=0 claim, **not yet verified by the bridge** | ansatz confirms |
...
| R-5 | G-6 | **resolved — corrected** | cuspis positions it in `RESULT.md` |
| R-6 | quantum holds a copy of cuspis's solver | **already recorded** — A4 DAG edge 16 | — |
| R-7 | B-9 / B-11 not in ansatz's list | open | ansatz scores both |
| R-8 | B-16 not in deepstrain's list | open | deepstrain scores it |
| **R-9** *(new)* | ansatz #1 "none known in any dimension" contradicted by EXP-002 | open | ansatz corrects its headline, as the bridge did |
