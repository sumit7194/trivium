...
forces the orbit average of the right-hand side to vanish — a necessary condition **on `F0` alone**. If
`F0` ranges over the polynomial algebra generated by the isometries and `Q`, that average factors as
`(∂F0/∂Q)·A`, with `A = ⟨{H1,Q}⟩` the rank-2 source alone. `A ≠ 0` on an open set then kills every `F0`
containing `Q` **at all ranks at once**.

### PRIOR-ART GATE — verdict: PARTIAL, and the novel half is the smaller half

    the averaging step            CLASSICAL.  The homological equation and its solvability condition
                                  (RHS must average to zero over the unperturbed torus) is standard
                                  canonical perturbation theory / normal forms, and is the first step
                                  of Poincare's non-existence theorem for analytic integrals (1899).
    polynomial-in-momenta         STUDIED.  Kozlov's obstructions to integrable geodesic flows, and
      integrals as a target       Kolokoltsov's extension to integrals polynomial in momenta. Those
                                  results are topological (genus) rather than perturbative on a fixed
                                  4D metric, so the setting differs -- the target class does not.
    the CONCLUSION for sGB        ALREADY ARGUED, TWICE.  Owen-Yunes-Witek (arXiv:2103.15891) solve the
                                  Killing equation to rank 2 in sGB and "conjecture, though we cannot
                                  prove" that no Killing tensor of any rank exists. Deich,
                                  Cardenas-Avendano & Yunes (arXiv:2203.00524, "Chaos in Quadratic
                                  Gravity") independently argue the same conclusion from POINCARE
                                  SECTIONS showing chaotic features.
    the dF0/dQ factorisation      NO DIRECT HIT.  This is the surviving candidate for novelty.

> **The reframing this forces, and it must happen BEFORE the compute, not after.** The contribution on
> offer is **not** "sGB has no fourth constant" — that is conjectured in the literature and supported
> numerically. It is **closing a stated conjecture by an exact method**, at first order in the coupling.
> That is a real and citable thing to do. It is a different paper from the one the briefing implied, and
> it changes what has to be measured.

*Caution on the surviving half: once the orbit average is written as a derivation on the polynomial
algebra, the `∂F0/∂Q` factorisation may be immediate. A referee could call it a remark rather than a
theorem. It should be written up as the mechanism that makes one rank-2 computation sufficient, not as a
new obstruction technique.*

*Second caution, unprompted by the proposal: the argument's entire content sits in the hypothesis that
`F1` is **bounded** along the orbit. For a merely formal first-order integral that is an assumption, not
a consequence. State it as a hypothesis or the obstruction proves nothing.*

### PRE-REGISTERED BUCKETS — frozen before `A` is computed

    A != 0 on an open set of      OBSTRUCTION FIRES. Every F0 containing Q dies at all ranks, at first
      inclined orbits             order in the coupling. Upgrades the OYW conjecture to a theorem at
                                  that order. Report as such -- NOT as a discovery of non-integrability.

    A == 0 identically            INCONCLUSIVE. NOT a survival. This is the bucket most likely to be
                                  misread and it is the reason this file exists.

    A == 0 AND the controls       INSTRUMENT FAILURE, not physics. Equatorial orbits and within-family
      also return 0               Kerr reparametrisations must BOTH return zero by construction; if they
                                  are the only zeros observed, nothing has been measured.

    A != 0 but only at            INCONCLUSIVE. The claim is about an OPEN SET. An accidental zero at
      isolated orbits             one radius is not vanishing; a nonzero value at one radius is not
                                  an open set either.

**The literature cross-check that comes free, and that nobody asked for.** Deich et al. find chaos in
sGB numerically. **A well-formed obstruction should therefore FIRE on sGB.** If `A` comes back
identically zero there, that is not a quiet null — it is in tension with a published numerical result,
and the instrument is the first suspect. *This is a sharper falsifier than "inconclusive" and it exists
only because the gate was run first.*

**Withdrawal condition, stated now:** if the `∂F0/∂Q` factorisation turns out to be a one-line corollary
of writing the average as a derivation, the approach loses its claim to novelty and should be reported as
